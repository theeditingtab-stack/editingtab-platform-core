...
CORE-003 adds Core-owned organizations, users, and memberships. These are internal Python services and repositories only. No authentication, authorization, public management endpoints, passwords, login sessions, roles, invitations, Booking, or frontend are implemented.
...
- Slugs: trim surrounding whitespace, lowercase, then require 1–63 ASCII letters/digits separated by single hyphens. Names are trimmed, nonblank, at most 200 characters, and contain no control characters.
...
- Baseline → `0002_core_identity` → repeated head: passed within the isolated test database.
