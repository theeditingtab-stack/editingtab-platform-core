...
| Permissions | Deny by default; explicit permission catalog; organization-scoped role assignments; separate platform authority. Check membership, permission, and entitlement on protected operations. | CORE-005 implements the four-code catalog, custom roles, grant ceilings, current-state checks, and last-admin protection. CORE-006 adds separate platform grants and current-state module entitlements; no tenant role or session claim grants platform authority. |
...
| Role permissions | CORE-005 settles four organization permissions, grant ceilings, last-admin checks, and an explicit local owner role. CORE-006 settles initial platform onboarding and entitlements for existing credentialed owners. Future feature permissions, invitations, and production operational controls remain open. | Before invitations and additional feature endpoints; no self-escalation is negotiable. |
...
CORE-003 identity persistence is committed and verified. CORE-004 implements password authentication; CORE-005 implements organization authorization and role audit. CORE-006 implements separate platform authority, one-time operator bootstrap, atomic existing-user onboarding, and module entitlement endpoints. Recovery and invitations remain future work. See [CORE-004 policy and limitations](core-004-authentication.md). Provider access, domains, recovery, and future module implementation remain unresolved as listed. Stop after CORE-007; this is not production readiness.
