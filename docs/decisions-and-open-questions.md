...
- Remote GitHub repository status is not yet confirmed. Do not assume the old remote repository was deleted.
...
| Foundation | Typed environment configuration; ignored local secrets and a placeholder-only example; separate liveness and database readiness endpoints; migration-managed schema; disposable PostgreSQL for CI. Health responses expose no secrets. | Accepted and implemented in CORE-002; local verification passed and remote CI remains pending. |
| Tenant isolation | Organization ownership on tenant records; derive and validate organization context server-side, scope repository queries, and enforce constraints. A domain identifies context but does not authorize a user. Evaluate PostgreSQL row-level security as additional protection. | Finalize before CORE-003 schema/access work; row-level security decision must precede tenant query implementation. |
| Permissions | Deny by default; explicit permission catalog; organization-scoped role assignments; separate platform authority. Check membership, permission, and entitlement on protected operations. | Finalize before CORE-004. |
...
| Deletion | Record deletion state and metadata where appropriate; disable access without erasing history. Define retention and dependency rules separately. Do not expose generic reservation deletion as cancellation. | Finalize entity-specific behavior before related CRUD work. |
...
| Authentication | Managed identity or local credentials? Session/cookie or token model? Invitations, password recovery, verification, platform MFA, and initial platform operator bootstrap? Which outbound message provider is available? | Before CORE-003 and authentication-sensitive frontend work; can wait through foundation with no authentication implementation. |
| Tenant isolation | Organization context selection, membership lifecycle, tenant uniqueness rules, database isolation strategy, and treatment of cross-organization users? | Before CORE-003 schema and access implementation. |
| Role permissions | Permission catalog, role administration limits, delegated grants, default roles, and platform operator powers? | Before CORE-004; no self-escalation is negotiable. |
...
| Domains | Verification method, re-verification/ownership transfer, DNS/TLS provisioning, fallback domain, and trusted proxy configuration? | Before CORE-005 and live domain flows; does not block database connectivity. |
...
| Audit and retention | Which events, actor metadata, retention periods, access permissions, and permanent-deletion approval process are required? | Before CORE-005 and deletion behavior; permanent deletion stays unavailable until controlled requirements are agreed. |
| GitHub and CI | Does a remote repository already exist under the company owner, and what history/settings must be preserved? What repository and runner permissions are available? | Before user remote setup/push and remote CI execution. Local foundation and CI file preparation can proceed without assuming remote state. |
...
CORE-002 is implemented and locally verified without pre-empting authentication, tenant schema, provider access, or recovery choices. CORE-003 requires explicit authentication/bootstrap and organization-membership isolation decisions before domain implementation. No next checkpoint starts automatically. The backend foundation is not production readiness.
