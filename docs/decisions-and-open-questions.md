...
| Role permissions | CORE-005 settles four organization permissions, grant ceilings, last-admin checks, and an explicit local owner role. Platform operator powers, future feature permissions, and production onboarding remain open. | Before platform administration, onboarding, and additional feature endpoints; no self-escalation is negotiable. |
...
| GitHub and CI | CORE-002 CI passed according to the user. CORE-003 is committed and verified according to the user. CORE-004 is verified at `aedfa62`; CORE-005 remote verification awaits manual push; visibility is handled separately. | Does not block local implementation. Do not modify GitHub settings. |
...
CORE-003 identity persistence is committed and verified. CORE-004 implements password authentication; CORE-005 implements organization authorization and role audit. Platform bootstrap, onboarding, and entitlements remain required for their future endpoints. See [CORE-004 policy and limitations](core-004-authentication.md). Provider access, domains, entitlements, recovery, and future modules remain unresolved as listed. Stop after CORE-005; this is not production readiness.
