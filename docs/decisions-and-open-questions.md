...
| Authentication | Managed identity or local credentials? Session/cookie or token model? Invitations, password recovery, verification, platform MFA, and initial platform operator bootstrap? Which outbound message provider is available? | Before the separately assigned authentication checkpoint and public management endpoints; not a blocker for completed internal persistence. |
...
| GitHub and CI | CORE-002 CI passed according to the user. CORE-003 remote verification awaits the user's manual push; visibility is handled separately. | Does not block local implementation. Do not modify GitHub settings. |
...
CORE-003 implements only internal identity persistence and is locally verified. Authentication, authorization, and platform bootstrap must be resolved before public management endpoints. Provider access, domains, entitlements, recovery, and future modules remain unresolved as listed. Stop after CORE-003; this is not production readiness.
