...
CORE-002 provides the backend foundation; the user reports that its local checks and GitHub Actions passed. CORE-003 adds internal organization, user, and membership persistence, with scoped queries, reserved uniqueness, and soft deletion. See [CORE-003 identity](core-003-identity.md) for policies and verification. Persistence scoping does not provide authentication or authorization. These persistence safeguards are now complemented by CORE-004 authentication and CORE-005 organization authorization.

CORE-004 adds password authentication with revocable server-side sessions, Origin-based CSRF protection, shared PostgreSQL login throttling, and explicit local-only provisioning. See [authentication details](core-004-authentication.md). CORE-005 adds the explicit four-permission catalog, custom roles, scoped management API, last-administrator protection, and transactional role audit. See [authorization details](core-005-authorization.md). CORE-006 adds separate platform grants, company-controlled existing-user onboarding, and current-state module entitlements. Only Booking entitlement activation is supported; Booking itself remains unimplemented. See [platform administration](core-006-platform.md). Invitations, recovery, domains, and all remaining product requirements stay visible; none have been removed.

CORE-007 adds Core's side of a versioned internal authorization connection to the separate Booking application/database. It validates both service and user identity plus current organization permissions and entitlement. CORE-AUTH-002 separates use from explicit delegation and enforces granular role operations. It does not implement the Booking client or inventory. See the [contract and explicit permission delegation policy](booking-authorization-contract.md).
